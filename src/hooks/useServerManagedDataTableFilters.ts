...
export type ServerManagedFilterDefinition = SingleFilterDefinition | MultiFilterDefinition | RangeFilterDefinition;
...
export const serverManagedFilter = {
	...
	range: (filterId: string, queryKey: string = filterId, operators?: RangeOperator[]): RangeFilterDefinition => ({
		filterId,
		type: "range",
		queryKey,
		operators,
		...
		(filterId: string, value: DataTableFilterValue | undefined) => {
			...
			updateParams(
				(params) => {
					...
					const rangeValue =
						value && !Array.isArray(value) && typeof value === "object" ? (value as DataTableRangeValue) : {};
