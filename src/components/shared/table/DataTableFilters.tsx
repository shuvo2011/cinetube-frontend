...
export type DataTableFilterConfig = SingleSelectFilterConfig | MultiSelectFilterConfig | RangeFilterConfig;
...
const MultiSelectFilterControl = ({
	...
	const applyNow = () => {
		onFilterChange(filter.id, selectedValues.length > 0 ? selectedValues : undefined);
	};
	...
	return (
		<div className="space-y-3">
			...
			<div className="flex items-center justify-between gap-2">
				<Button type="button" size="sm" variant="ghost" onClick={() => setSelectedValues([])} disabled={isLoading}>
					Clear
				</Button>
...
const RangeFilterControl = ({
	...
	return (
		<div className="space-y-3">
			...
			<div className="flex items-center justify-between gap-2">
				<Button type="button" size="sm" variant="ghost" onClick={clearRange} disabled={isLoading}>
					Clear
				</Button>
...
const DataTableFilters = ({ filters, values, onFilterChange, onClearAll, isLoading }: DataTableFiltersProps) => {
		...
		<div className="space-y-2">
			<div className="flex flex-wrap items-center gap-2">
				{filters.map((filter) => {
					...
					const triggerClass = cn("h-9", activeCount > 0 && "border-primary text-primary");
				...
				{onClearAll && totalActiveFilters > 0 && (
					<Button type="button" variant="ghost" className="h-9" onClick={onClearAll} disabled={isLoading}>
						<X className="h-4 w-4" />
						Clear Filters
					</Button>
				)}
			...
			{activeBadges.length > 0 && (
				<div className="flex flex-wrap gap-1.5">
					{activeBadges.map((badge) => (
						<Badge key={badge.key} variant="secondary" className="flex items-center gap-1 pr-1 text-xs">
							{badge.label}
							<button
								type="button"
